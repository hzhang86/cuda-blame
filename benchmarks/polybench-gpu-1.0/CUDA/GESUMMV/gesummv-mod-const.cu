...
__constant__ DATA_TYPE x_gpu[N];
...
__global__ void gesummv_kernel(DATA_TYPE *a, DATA_TYPE *b, /*DATA_TYPE *x, */DATA_TYPE *y/*, DATA_TYPE *tmp*/)
{
	...
	{
		int j;
        float tmp = 0.0, y_tmp = 0.0;
		for(j = 0; j < N; j++)
		{	
			//tmp[i] += a[i * N + j] * x[j];
			tmp += a[i * N + j] * x_gpu[j];
			//y[i] += b[i * N + j] * x[j];
			y_tmp += b[i * N + j] * x_gpu[j];
		}
		//y[i] = ALPHA * tmp[i] + BETA * y[i];
		y[i] = ALPHA * tmp + BETA * y_tmp;
	}
}
...
void gesummvCuda(DATA_TYPE* A, DATA_TYPE* B, DATA_TYPE* x, DATA_TYPE* y, /*DATA_TYPE* tmp, */DATA_TYPE* y_outputFromGpu)
{
	...
	cudaMemcpyAsync(A_gpu, A, sizeof(DATA_TYPE) * N * N, cudaMemcpyHostToDevice);
	cudaMemcpyAsync(B_gpu, B, sizeof(DATA_TYPE) * N * N, cudaMemcpyHostToDevice);
	//cudaMemcpy(x_gpu, x, sizeof(DATA_TYPE) * N, cudaMemcpyHostToDevice);
	cudaMemcpyToSymbol(x_gpu, x, sizeof(DATA_TYPE) * N);
    cudaMemcpyAsync(y_gpu, y, sizeof(DATA_TYPE) * N, cudaMemcpyHostToDevice);
