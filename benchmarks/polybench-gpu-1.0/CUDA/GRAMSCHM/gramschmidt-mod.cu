...
__global__ void gramschmidt_kernel3(DATA_TYPE *a, DATA_TYPE *r, DATA_TYPE *q, int k)
{
    ...
    __shared__ DATA_TYPE sq_tmp[M]; //shared q tmp
    
    int si, idx;
    int stride = M/blockDim.x; //here stride = 8 (2048/256)
    for (si = 0; si < stride; si++) {
        idx = si * blockDim.x + threadIdx.x;
        sq_tmp[idx] = q[idx*N + k];
    }
    __syncthreads();
